...
public interface VentaRepository extends JpaRepository<Venta, Integer> {
    ...
    @Query(value = """
        ...
        FROM oms.venta_detalle vd
        JOIN oms.ventas v ON vd.venta_id = v.id
    ...
    @Query(value = """
        SELECT c.nombre 
        FROM oms.clientes c
        JOIN oms.ventas v ON v.cliente_id = c.id
        JOIN oms.venta_detalle d ON d.venta_id = v.id
        GROUP BY c.id
    ...
    @Query(value = """
        ...
        FROM oms.productos p
        JOIN oms.venta_detalle d ON d.producto_id = p.id
        GROUP BY p.id
